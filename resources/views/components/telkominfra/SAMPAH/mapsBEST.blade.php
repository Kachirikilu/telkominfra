...
<div class="mt-8">
    ...
    <div class="mb-4 flex flex-wrap gap-2">
        <button type="button" data-value="rsrp"
            class="metric-btn metric-btn-active w-1/2 sm:w-auto bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-1 rounded-lg transition duration-150 shadow-md">
            RSRP
        </button>
        <button type="button" data-value="rssi"
            class="metric-btn w-1/2 sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-1 rounded-lg transition duration-150 shadow-md">
            RSSI
        </button>
        <button type="button" data-value="rsrq"
            class="metric-btn w-1/2 sm:w-auto bg-orange-500 hover:bg-orange-600 text-white font-semibold px-6 py-1 rounded-lg transition duration-150 shadow-md">
            RSRQ
        </button>
        <button type="button" data-value="sinr"
            class="metric-btn w-1/2 sm:w-auto bg-red-600 hover:bg-red-700 text-white font-semibold px-6 py-1 rounded-lg transition duration-150 shadow-md">
            SINR
        </button>
    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function getColorByRSSI(rssi) {
            if (rssi >= -72) return '#0051ff';
            if (rssi >= -78) return '#16bef7';
            if (rssi >= -84) return '#00ffc8';
            if (rssi >= -90) return '#2ECC71';
            if (rssi >= -96) return '#F4D03E';
            if (rssi >= -102) return '#FF8C00';
            if (rssi >= -108) return '#FF4500';
            if (rssi >= -114) return '#d82a17';
            if (rssi > -120) return '#800000';
            if (rssi <= -120) return '#000000';
            return '#C0C0C0';
        }
            ...
            legend.onAdd = function(map) {
                ...
                for (let i = 0; i < colors.length; i++) {
                    div.innerHTML +=
                        ...
                        '; width: 12px; height: 12px; float:left; margin-right:6px;"></i> ' +
