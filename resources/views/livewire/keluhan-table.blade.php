                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($keluhans as $item)
                                @php
                                    ...
                                    $shortKomentar = $item->komentar ? (strlen($item->komentar) > 25 ? substr($item->komentar, 0, 25) . '...' : $item->komentar) : '-';
                                @endphp

                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $item->id }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $item->user->name ?? $item->nama_pengguna ?? '-' }}</td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="flex items-center justify-center space-x-2">
                                            ...
                                            @if ($isAdmin || $item->user_id == $idUser)
                                                <form method="POST" action="{{ route('keluh_pengguna.destroy', $item->id) }}"
